# Fonts — licenses and attribution

The WOFF2 files in this directory are redistributed subject to the terms of
the **SIL Open Font License 1.1**. Copyright notices for each family
follow, then the full OFL text.

Only the `latin` and `latin-ext` subsets are bundled; other subsets
(cyrillic, greek, vietnamese, …) are omitted for size, not licensing
reasons — the same OFL terms would apply to those subsets as well if
added.
...
Licensed under the SIL Open Font License, Version 1.1. Upstream:
<https://rsms.me/inter/>.
