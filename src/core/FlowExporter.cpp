...
namespace core {
    ...
    void FlowExporter::writeJson(const std::vector<Flow>& flows, std::ostream& out,
        ...
        document += ",\n  \"generated_unix_us\": ";
        document += std::to_string(currentUnixTimeMicroseconds());
        document += ",\n  \"flow_count\": ";
        document += std::to_string(flows.size());
        document += ",\n  \"flows\": [";
        ...
        for (const auto& flow : flows) {
            ...
            document += ",\n      \"src_port\": ";
            document += std::to_string(flow.key.srcPort);
            document += ",\n      \"dst_ip\": ";
            appendJsonString(document, flow.key.dstIP);
            document += ",\n      \"dst_port\": ";
            document += std::to_string(flow.key.dstPort);
            document += ",\n      \"protocol\": ";
            ...
            document += ",\n      \"asn\": ";
            document += std::to_string(info.asn);
            document += ",\n      \"organization\": ";
            appendJsonString(document, info.organization);
            document += ",\n      \"packets\": ";
            document += std::to_string(flow.packets);
            document += ",\n      \"bytes_up\": ";
            document += std::to_string(flow.bytesUp);
            document += ",\n      \"bytes_down\": ";
            document += std::to_string(flow.bytesDown);
            document += ",\n      \"first_seen_us\": ";
            document += std::to_string(flow.firstSeen);
            document += ",\n      \"last_seen_us\": ";
            document += std::to_string(flow.lastSeen);
            document += ",\n      \"duration_sec\": ";
            document += std::to_string(durationSecondsOf(flow));
            document += ",\n      \"rate_bytes_per_sec\": ";
            document += formatFloat(flow.rateBytesPerSecond, 1);
            document += ",\n      \"initial_rtt_us\": ";
            document += std::to_string(flow.initialRttMicroseconds);
            document += ",\n      \"retransmissions_up\": ";
            document += std::to_string(flow.retransmissionsUp);
            document += ",\n      \"retransmissions_down\": ";
            document += std::to_string(flow.retransmissionsDown);
            document += ",\n      \"out_of_order_up\": ";
            document += std::to_string(flow.outOfOrderUp);
            document += ",\n      \"out_of_order_down\": ";
            document += std::to_string(flow.outOfOrderDown);
            document += ",\n      \"encrypted_tunnel\": ";
