...
namespace capture {
    namespace {
        class NpcapBackend final : public ICaptureBackend {
        public:
            ~NpcapBackend() override { close(); }

            std::vector<DeviceInfo> listDevices(std::string& error) const override {
                std::vector<DeviceInfo> devices;
                pcap_if_t* allDevices = nullptr;
                char errorBuffer[PCAP_ERRBUF_SIZE]{};
                if (pcap_findalldevs_ex(PCAP_SRC_IF_STRING, nullptr, &allDevices, errorBuffer) == -1) {
                    error = errorBuffer;
                    return devices;
                }
    ...
    std::unique_ptr<ICaptureBackend> createPlatformCaptureBackend() {
        return std::make_unique<NpcapBackend>();
    }
} // namespace capture
